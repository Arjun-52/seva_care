...
class EditSeniorScreen extends StatefulWidget {
  final SeniorDetailsModel senior;
...
class _EditSeniorScreenState extends State<EditSeniorScreen> {
  ...
  void initState() {
    ...
    _phoneController = TextEditingController(text: widget.senior.phone ?? '');
    _cityController = TextEditingController(text: widget.senior.city);
    _conditionsController = TextEditingController(text: widget.senior.conditions.join(', '));
    _addressController = TextEditingController(text: widget.senior.address ?? '');
    _emergencyNotesController = TextEditingController(text: widget.senior.emergencyNotes ?? '');

    _gender = ['male', 'female', 'other'].contains(widget.senior.gender.toLowerCase())
        ? widget.senior.gender.toLowerCase()
        : 'male';
    _tier = ['Care Lite', 'Care Plus', 'Care Premium'].contains(widget.senior.tier)
        ? widget.senior.tier
        : 'Care Plus';
    _status = ['stable', 'monitoring', 'critical'].contains(widget.senior.status.toLowerCase())
        ? widget.senior.status.toLowerCase()
        : 'stable';
    _mobility = ['independent', 'assisted', 'wheelchair', 'bedridden'].contains(widget.senior.mobility.toLowerCase())
        ? widget.senior.mobility.toLowerCase()
        : 'independent';
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      final updated = await locator<SeniorRepository>().updateSenior(widget.senior.id, payload);
      AppLogger.i('Senior updated successfully: ${updated.id}');
      ...
      AppLogger.e('Update senior failed', e, stack);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Edit Senior Details'),
        leading: IconButton(
